...
export const archiveSession = (active: ActiveStudySession, reviewAtEnd: boolean, timerMs: number|null): StudySession => {
  ...
  return { id: active.id, type:active.type, source:active.source, startedAt:active.startedAt, finishedAt:Date.now(), shuffled:false, timerMs, reviewMistakesAtEnd:reviewAtEnd, deckOrder:active.deck.map(entry=>entry.kanaId), results:[...map].map(([kanaId,row])=>({kanaId,...row})) }
}
