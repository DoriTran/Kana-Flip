...
export type StudySource = Alphabet | 'both' | 'starred' | 'recent-mistakes' | 'session-mistakes'
...
export interface StudyPreferences { characterSet: 'hiragana' | 'katakana' | 'both'; reviewMode: 'none' | 'starred' | 'recent-mistakes'; reviewMistakesAtEnd: boolean; shuffled: boolean; lockNavigation: boolean; timerMs: number | null; recordSession: boolean; readyFirstCard: boolean; showKeyboardHints: boolean; reducedMotion: boolean }
...
export interface ActiveStudySession { id: string; type: SessionType; source: StudySource; startedAt: number; deck: DeckEntry[]; currentIndex: number; flipped: boolean; reviewQueue: string[]; reviewIndex: number; reviewPhase: boolean; reviewTimings: Record<string, number>; timerRemainingMs: number | null; timerStartedAt: number | null; recordSession: boolean; waitingToStart: boolean; completed: boolean }
