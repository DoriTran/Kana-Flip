import { BookOpenCheck, Clock3, Flower, RotateCcw, Shuffle, Trash2 } from 'lucide-react'
        ...
        {sessions.length === 0
          ...
          : sessions.map(session => {
            ...
            return <article key={session.id} className={session.type === 'review' ? 'review-row' : ''}>
              <span className="history-kana">{session.source === 'both' ? <Flower aria-label="Both" /> : session.source === 'katakana' ? 'ア' : session.source === 'hiragana' ? 'あ' : <BookOpenCheck aria-label="Review" />}</span>
              <div><b className="session-title-row">{session.type === 'review' ? 'Review · ' + sourceLabel(session.source) : sourceLabel(session.source)}{session.reviewMistakesAtEnd && <RotateCcw aria-label="Review mistakes at end" />}{session.shuffled && <Shuffle aria-label="Shuffled" />}{session.timerMs != null && <Clock3 aria-label="Timer on" />}</b><small>{new Date(session.finishedAt).toLocaleString()}</small></div>
              {session.type === 'study'
